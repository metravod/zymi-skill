...
- **[references/pipelines.md](references/pipelines.md)** — pipeline YAML schema, the three step kinds (agent / deterministic tool / ask), `when:` branching, `any_of` outputs, fresh context, fork/resume. Load when the user is designing or editing a pipeline.
...
- **[references/zymi-as-mcp-server.md](references/zymi-as-mcp-server.md)** — `zymi mcp serve` (zymi as MCP server): `expose.mcp:`, host wiring, SEP-1686 tasks for long pipelines, elicitation approval forms, reasoning delegation (`ask:` steps → `input_required` + `zymi/reasoning/resume`), `zymi.runs.*` introspection tools. Load when another agent should call zymi pipelines.
...
| Ask the caller (or a human) to reason mid-pipeline, no second model | Ask step (`ask: "<prompt>"`) — parks and resumes with the caller's answer; under `zymi mcp serve` the connected agent answers via `zymi/reasoning/resume`, else a human at the terminal. No `llm:` needed (ADR-0042) |
